...
const ReportAnalysis = () => {
  ...
  const PIXABAY_API_KEY = "49856975-b50f4f2288e42fd6f79dc9c5d";

  const fetchImagesFromPixabay = async (keywords) => {
    ...
    try {
      ...
      // ✅ Return only 2 images max (1 or 0 if fewer available)
      const images = data?.hits?.slice(0, 4).map((img) => img.webformatURL) || [];
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch("https://rohit-mewada-1125-sympcare-models.hf.space/analyze-report", {
        method: "POST",
        body: formData,
      });
      ...
      const imgs = await fetchImagesFromPixabay(data.image_keywords);
      setImages(imgs);
  ...
  const sendMessage = async (message) => {
    ...
    try {
      const response = await fetch("https://rohit-mewada-1125-sympcare-models.hf.space/report-chat", {
        method: "POST",
        ...
        <div className="flex flex-col lg:flex-row gap-8 mt-10">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto bg-blue-50 p-4 rounded-2xl flex flex-col space-y-3">
              {messages.map((msg, index) => (
                ...
                >
                  <div
                    className={`max-w-[80%] p-3 rounded-2xl shadow-md break-words ${msg.sender === "bot"
            ...
            <div className="p-3 border-t bg-white flex items-center space-x-2">
              ...
              <button
                ...
                className={`ml-2 px-4 py-2 rounded-lg ${isListening ? "bg-red-500" : "bg-blue-600"
                  } text-white`}
              >
            ...
            <div className="mt-3 flex justify-center">
              <button
                onClick={() =>
                  sendMessage("can you give Get Lifestyle Recommendations in only 100 words")
                }
          ...
          >
            ...
            <h3 className="text-xl font-bold text-blue-700 mb-1">
              ...
              {/* Short Disclaimer */}
            <p className="text-xs text-red-500 mb-2 italic text-center">
              *Images are for reference only and may not be medically exact.*
            </p>
